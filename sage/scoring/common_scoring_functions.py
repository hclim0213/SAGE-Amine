...
from sage.utils.descriptors import mol_weight, logP, num_H_donors, tpsa, num_atoms, AtomCounter
from sage.utils.fingerprints import get_fingerprint
...
from sage.utils.chemistry import smiles_to_rdkit_mol, parse_molecular_formula
from sage.utils.math import arithmetic_mean, geometric_mean
...
class RdkitScoringFunction(ScoringFunctionBasedOnRdkitMol):
    ...
    def score_batch(self, smiles_list: List[str]):
        list_mol = [mol for mol in (smiles_to_rdkit_mol(smiles) for smiles in smiles_list) if mol is not None]
        return self.score_mol_batch(list_mol)
    
    def score_mol_batch(self, mol: Union[Chem.Mol, List[Chem.Mol]]):
        return self.descriptor(mol)
