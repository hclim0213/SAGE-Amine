...
from sage.models.apprentice import (
    AbstractGenerator,
    TransformerDecoderGenerator,
)
from sage.models.handlers.abstract_handler import AbstractGeneratorHandler
from sage.utils.smiles import smiles_to_actions


class TransformerDecoderGeneratorHandler(AbstractGeneratorHandler):
    def __init__(
        self,
        model: TransformerDecoderGenerator,
        optimizer,
    ...
    def valid_on_action_batch(
        ...
        output = self.model(tgt=input_actions, use_tgt_mask=True)
    ...
    def _sample_action_batch(self, batch_size: int, device: torch.device):
        ...
        for step in range(self.max_seq_length):
            logits = self.model(tgt=inp, use_tgt_mask=True)
