...
def canonicalize_and_score_smiles(
    ...
    elif evaluation_batch_size > 1:
        canon_results = Parallel(n_jobs=int(pool.n_jobs))(
            delayed(scoring_function.score_batch)(temp_batch) 
            for temp_batch in chunked_iterable(canon_smiles, evaluation_batch_size)
        )
        
        flattened_smiles = []
        flattened_scores = []
        
        for item in canon_results:
            if isinstance(item[0], list) and isinstance(item[1], list):
                flattened_scores.extend(item[0])
                flattened_smiles.extend(item[1])
            elif isinstance(item[0], float) and isinstance(item[1], str):
                flattened_scores.append(item[0])
                flattened_smiles.append(item[1])
        
        canon_scores = flattened_scores
        canon_smiles = flattened_smiles
