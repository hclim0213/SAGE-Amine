...
from sage.benchmark.benchmarks_opt_amine import load_benchmark as load_benchmark_amine
from sage.benchmark.benchmarks_opt_drug import load_benchmark as load_benchmark_drug
from sage.benchmark.benchmarks_opt_sage import load_benchmark as load_benchmark_sage
...
if __name__ == "__main__":
    ...
    early_stop = False
    if args.benchmark_type == 'drug':
        benchmark, scoring_num_list = load_benchmark_drug(args.benchmark_id)
    
    if args.benchmark_type == 'sage':
        benchmark, scoring_num_list = load_benchmark_sage(args.benchmark_id)
        
        if args.benchmark_id <= 20:
            early_stop = True
        
        if args.benchmark_id >= 29 and args.benchmark_id <= 56:
            early_stop = True

    if args.benchmark_type == 'amine':
        benchmark, scoring_num_list = load_benchmark_amine(args.benchmark_id)
        
        if args.benchmark_id <= 26:
            early_stop = True
    ...
    initial_smiles = None
    if args.use_initial or (args.model_type == 'Transformer'):
